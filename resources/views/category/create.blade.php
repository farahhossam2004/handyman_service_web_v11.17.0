    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        {{ html()->form('POST', route('category.store'))->attribute('enctype', 'multipart/form-data')->attribute('data-toggle', 'validator')->attribute('data-language-required-mode', 'active')->id('category-form')->open() }}
                                        ...
                                        <div class="form-group col-md-{{ $field === 'name' ? '4' : '12' }}">
                                            ...
                                            @php
                                                ...
                                                if ($categorydata && $categorydata->id) {
                                                    $translation = $categorydata->translations()
                                                ...
                                                if ($categorydata && isset($categorydata->id)) {
                                                    $mainValue = $categorydata->getAttribute($field) ?? '';
                                                }
                                                
                                                // Determine display value:
                                                // - English tab  → main column value (always English)
                                                // - Other tabs   → translation row for that locale
                                                if ($language['id'] === 'en') {
                                                    $value = $mainValue;
                                                ...
                                                // Field name rules:
                                                // - English always posts as the bare field (e.g. "name") → stored in main column
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <div class="custom-file">
                                ...
                                <input type="file" name="category_image" class="custom-file-input"
                                    ...
                                    {{ is_null($categorydata->id) ? 'data-file-required="true"' : '' }}>
                                ...
                                @foreach ($language_array as $language)
                                    <div id="seo-form-language-{{ $language['id'] }}" class="language-form"
                                        ...
                                        <div class="form-group col-md-6 mb-3">
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <span class="text-muted" style="font-size: 12px;">
                                                    @php
                                                        $metaTitleVal = $categorydata && $categorydata->id
                                                            ? ($language['id'] === 'en'
                                                                ? ($categorydata->getAttribute('meta_title') ?? '')
                                                                : ($categorydata->translations()->where('attribute','meta_title')->where('locale',$language['id'])->value('value') ?? ''))
                                                            : '';
                                                    @endphp
                                                    <span
                                        ...
                                        <div class="form-group col-md-6 mb-3">
                                            ...
                                            @php
                                                $metaKeywordsVal = $categorydata && $categorydata->id
                                                    ? ($language['id'] === 'en'
                                                        ? (is_array($categorydata->getAttribute('meta_keywords'))
                                                            ? implode(',', $categorydata->getAttribute('meta_keywords'))
                                                            : ($categorydata->getAttribute('meta_keywords') ?? ''))
                                                        : ($categorydata->translations()->where('attribute','meta_keywords')->where('locale',$language['id'])->value('value') ?? ''))
                                                    : '';
                                        ...
                                        <div class="form-group col-12 mb-3">
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <span class="text-muted" style="font-size: 12px;">
                                                    @php
                                                        $metaDescVal = $categorydata && $categorydata->id
                                                            ? ($language['id'] === 'en'
                                                                ? ($categorydata->getAttribute('meta_description') ?? '')
                                                                : ($categorydata->translations()->where('attribute','meta_description')->where('locale',$language['id'])->value('value') ?? ''))
                                                            : '';
                                                    @endphp
                                                    <span
    ...
    @section('bottom_script')
        <script type="text/javascript">
            ...
            function checkImage() {
                ...
                var imageInput = document.querySelector('input[name="category_image"]');
                var saveButton = document.getElementById('saveButton');

                // For create flow (no ID), rely on selected file only.
                if (!id) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        var hasSelectedImage = imageInput && imageInput.files && imageInput.files.length > 0;

                        if (attachmentsCount == 0 && !hasSelectedImage) {
                            $('input[name="category_image"]').attr('required', 'required');
                            saveButton.disabled = false;
                        } else {
                            $('input[name="category_image"]').removeAttr('required');
