...
<div class="col-md-12">
    <div class="row ">
        ...
        <div class="col-md-9">
            ...
            {{ html()->hidden('email') }}
            {{ html()->hidden('user_type', $user_data->user_type) }}
            {{ html()->hidden('id', $user_data->id ?? null)
                ...
                <div class="row ">
                    ...
                    @endif
                    @php
                        $__nbRow = \App\Models\Setting::where('type','OTHER_SETTING')->where('key','OTHER_SETTING')->first();
                        $__nbData = $__nbRow ? json_decode($__nbRow->value) : null;
                        $__nearbyEnabled = $__nbData && !empty($__nbData->nearby_provider);
                        $__isProvider = isset($user_data->user_type) && $user_data->user_type === 'provider';
                        $__addressRequired = $__isProvider && $__nearbyEnabled;
                    @endphp
                    <div class="form-group col-md-12">
                        {{ html()->label(__('messages.address') . ($__addressRequired ? ' <span class="text-danger">*</span>' : ''), 'address')->class('form-control-label') }}
                        @if($__addressRequired)
                            {{ html()->textarea('address', $user_data->address)->class('form-control textarea')->rows(2)->id('address')->placeholder(__('messages.address'))->required() }}
                        @else
                            {{ html()->textarea('address', $user_data->address)->class('form-control textarea')->rows(2)->id('address')->placeholder(__('messages.address')) }}
                        @endif
                        {{ html()->hidden('latitude', $user_data->latitude)->id('latitude') }}
                        {{ html()->hidden('longitude', $user_data->longitude)->id('longitude') }}
                    </div>
                ...
                <div class="form-group col-md-12 mt-4">
                    <h4>{{ __('messages.known_languages') }}</h4>
                </div>

                @php
                    $knownLanguages = is_array($user_data->known_languages) ? $user_data->known_languages : (is_string($user_data->known_languages) ? json_decode($user_data->known_languages, true) : []);
                    $knownLanguages = $knownLanguages ?: [''];
                @endphp

                @if(count($knownLanguages) > 0 && $knownLanguages[0] !== '')
                    @foreach($knownLanguages as $index => $language)
                        <div class="form-language-section form-group col-md-12">
                            <div class="row">
                                <div class="form-group col-md-12 d-flex">
                                    {{ html()->text('known_languages[]', $language)
                                        ->placeholder(__('messages.enter_language'))
                                        ->class('form-control')
                                    }}
                                    <small class="help-block with-errors text-danger"></small>
                                    <div class="form-group col-3 mb-0 align-self-center">
                                        <button type="button" class="remove-language button-custom button-remove" data-title="remove" title="Remove">
...
$(document).ready(function() {
    ...
    if (typeof PhoneInputHandler !== 'undefined' && typeof window.intlTelInput !== 'undefined') {
     ...
     $(document).ready(function () {
        ...
        // Remove Section1 for existing reasons
        $(document).on('click', '.remove-section1', function () {
            $(this).closest('.form-section1').remove();
        });

        // Function to update Remove button visibility for reasons
    ...
    $(document).ready(function () {
        // Initially hide Remove button if there's only one language section
        // updateLanguageRemoveButtonVisibility();
    });
    ...
    // Known Languages functionality
    $(document).on('click', '#add-language', function() {
        var newLanguageSection = $(".form-language-section:first").clone();
        newLanguageSection.find('input').val('');
        $(".form-language-section:last").after(newLanguageSection);
        updateLanguageRemoveButtonVisibility();
    });

    $(document).on('click', '.remove-language', function() {
        if ($(".form-language-section").length > 1) {
            $(this).closest('.form-language-section').remove();
            updateLanguageRemoveButtonVisibility();
        }
    });

    function updateLanguageRemoveButtonVisibility() {
        if ($(".form-language-section").length > 1) {
            $('.remove-language').show();
        } else {
