...
{{ html()->hidden('type', $page)
    ...
    <div class="form-group">
        <div class="form-control d-flex align-items-center justify-content-between">
            <label for="nearby_provider" class="mb-0">{{ __('messages.nearby_provider') }} <i class="fas fa-info-circle" style="cursor: help; margin-left: 5px; font-size: 14px;" data-bs-toggle="tooltip" data-bs-placement="top" title="Google Map key must be configured in the site setup with billing enabled."></i></label>
            <div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                <input type="checkbox" class="custom-control-input" name="nearby_provider" id="nearby_provider" {{ !empty($othersetting->nearby_provider) ? 'checked' : '' }}>
                <label class="custom-control-label" for="nearby_provider"></label>
            </div>
        </div>
    </div>
