...
<div class="row">
    <div class="col-lg-12">
        <div class="form-group">
            ...
            {{ html()->select('earning_type', ['commission' => __('messages.commission'),'subscription' => __('messages.subscription')], $earningsetting->value ?? 'commission')->id('earning_type')->class('form-select')->required() }}
        </div>
    </div>
    
    @php
        $autoFreeTrial = optional(json_decode(\App\Models\Setting::where('type', 'OTHER_SETTING')->where('key', 'OTHER_SETTING')->value('value'), true))['auto_assign_free_plan'] ?? 0;
        $currentEarningType = strtolower(trim($earningsetting->value ?? 'commission'));
    @endphp
    
    <div class="col-lg-12" id="auto-free-trial-section" style="display: {{ $currentEarningType === 'subscription' ? 'block' : 'none' }};">
        <div class="form-group">
            <div class="custom-control custom-switch">
                {{ html()->checkbox('auto_assign_free_plan', $autoFreeTrial == 1, 1)->class('custom-control-input')->id('auto_assign_free_plan') }}
                <label class="custom-control-label" for="auto_assign_free_plan">{{ __('messages.auto_free_trial') }}</label>
            </div>
            <small class="form-text text-muted">{{ __('messages.auto_free_trial_description') }}</small>
        </div>
    </div>
...
{{-- Toggle runs from parent page after AJAX load (see setting index & _dynamic_script) --}}
