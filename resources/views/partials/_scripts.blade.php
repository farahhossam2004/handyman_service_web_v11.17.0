...
<script>
      // The primary language for the main column is ALWAYS English.
   // app()->getLocale() is the UI locale (may be Dutch etc.) — do NOT use it here.
   ...
   function toggleLanguageForm(languageId) {
      ...
      // Hide all language forms and disable their inputs
      document.querySelectorAll('.language-form').forEach(function(form) {
         form.style.display = 'none';
         // Disable all inputs in hidden forms EXCEPT English (which always owns the bare field names)
         form.querySelectorAll('input, textarea, select').forEach(function(input) {
            const formLanguageId = form.id.replace('form-language-', '').replace('seo-form-language-', '');
            if (formLanguageId !== 'en') {
               input.disabled = true;
            }
         });
      });

      // Display the selected language form and enable its inputs
      const selectedForm = document.getElementById('form-language-' + languageId);
      const seoSelectedForm = document.getElementById('seo-form-language-' + languageId);
      if (selectedForm) {
         selectedForm.style.display = 'block';
         // Enable all inputs in the visible form
         selectedForm.querySelectorAll('input, textarea, select').forEach(function(input) {
            input.disabled = false;
         });
      }
      if (seoSelectedForm) {
         seoSelectedForm.style.display = 'block';
         // Enable all inputs in the visible SEO form
         seoSelectedForm.querySelectorAll('input, textarea, select').forEach(function(input) {
            input.disabled = false;
         });
      }
      ...
      reinitTagifyFor(languageId);
   }


   function reinitTagifyFor(languageId) {
    const input = document.getElementById('meta_keywords_' + languageId);
    if (input && !input._tagify) { // Avoid reinitializing if already done
        new Tagify(input,{
                    delimiters: ",",
                    whitelist: [],
                    dropdown: { enabled: 0 },
   ...
   function getLanguageRequiredMode() {
      const languageForm = document.querySelector('form[data-language-required-mode]');
      return languageForm ? (languageForm.dataset.languageRequiredMode || 'default') : 'default';
   }

   function getTargetLanguageId(currentLanguageId) {
      const mode = getLanguageRequiredMode();
      return mode === 'active' ? currentLanguageId : primaryLanguageId;
   }
   ...
   function setRequiredFields(languageId) {
    console.log(languageId, primaryLanguageId);
      const mode = getLanguageRequiredMode();
      const targetLanguageId = getTargetLanguageId(languageId);
      ...
      // In 'active' mode, show * markers on ALL language forms
      if (mode === 'active') {
         // Show * markers on all language forms
         document.querySelectorAll('.language-label span.text-danger').forEach(function (span) {
            span.style.display = 'inline';
         });
      } else {
         // In 'default' mode, only show * on target language
         document.querySelectorAll('.language-label span.text-danger').forEach(function (span) {
            span.style.display = 'none';
         });
         
         const activeLanguageForm = document.querySelector(`#form-language-${targetLanguageId}`);
      ...
      // Apply 'required' attribute for target language form (only in default mode)
      if (targetLanguageId && mode !== 'active') {
         const activeLanguageForm = document.querySelector(`#form-language-${targetLanguageId}`);
         if (activeLanguageForm) {
      ...
      // Handle SEO fields
      const seoActiveLanguageForm = document.querySelector(`#seo-form-language-${targetLanguageId}`);
      if (seoActiveLanguageForm) {
         var seoEnabledSwitch = document.getElementById('seo_enabled');
         if (seoEnabledSwitch) {
            seoEnabledSwitch.addEventListener('change', function () {
               seoActiveLanguageForm.style.display = this.checked ? 'block' : 'none';
               if (this.checked) {
                  seoActiveLanguageForm.querySelectorAll('[data-required="true"]').forEach(function(field) {
                     if (mode !== 'active') {
                        field.setAttribute('required', 'required');
                     }
                  });
                  seoActiveLanguageForm.querySelectorAll('.language-label span.text-danger').forEach(function (span) {
                     span.style.display = 'inline';
                  });
               } else {
                  seoActiveLanguageForm.querySelectorAll('[data-required="true"]').forEach(function(field) {
   ...
   function checkData() {
      let isValid = true;
      const mode = getLanguageRequiredMode();
      const activeLanguageButton = document.querySelector('.language-btn.btn-primary');
      const activeLanguageId =
         activeLanguageButton?.getAttribute('onclick')?.match(/'([^']+)'/)?.[1] || primaryLanguageId;

      if (mode === 'active') {
         // In 'active' mode, check the currently active/visible language tab
         const activeLanguageForm = document.querySelector(`#form-language-${activeLanguageId}`);
         
         if (activeLanguageForm && activeLanguageForm.style.display !== 'none') {
            const requiredFields = activeLanguageForm.querySelectorAll('[data-required="true"]');
            ...
            requiredFields.forEach(function(field) {
               ...
               if (!value.trim()) {
                  ...
                  const errorElement = field.parentElement.querySelector('.help-block.with-errors');
                  if (errorElement) {
                     errorElement.textContent = '{{ __("messages.field_required") }}';
            ...
            if (hasEmptyField) {
               ...
               showSnackbar('{{ __("messages.complete_required_fields", ["language" => "' + activeLanguageId.toUpperCase() + '"]) }}'.replace(':language', activeLanguageId.toUpperCase()));
            }
         }
      } else {
         // In 'default' mode, check only the primary language
         const targetLanguageId = getTargetLanguageId(activeLanguageId);
         const activeLanguageForm = document.querySelector(`#form-language-${targetLanguageId}`);
         ...
         if (activeLanguageForm) {
            ...
            requiredFields.forEach(function(field) {
               ...
               if (!value.trim()) {
                  isValid = false;
                  showSnackbar('{{ __("messages.field_required") }}');
                  field.focus();
      ...
      // Validate required file inputs (data-file-required="true")
      document.querySelectorAll('input[type="file"][data-file-required="true"]').forEach(function(fileInput) {
         if (!fileInput.files || fileInput.files.length === 0) {
            isValid = false;
            const errorElement = fileInput.closest('.form-group')?.querySelector('.help-block.with-errors, #category_image_error, #image_error');
            if (errorElement) {
               errorElement.textContent = '{{ __("messages.file_required") }}';
               errorElement.style.display = 'block';
            }
            fileInput.focus();
         }
   ...
   document.addEventListener('DOMContentLoaded', function() {
      ...
      // Disable inputs for language forms that are:
      //   - NOT English (English always submits its bare field names)
      //   - NOT the currently visible/active tab (the default language tab is shown on load)
      document.querySelectorAll('.language-form').forEach(function(form) {
         const languageId = form.id.replace('form-language-', '').replace('seo-form-language-', '');
         const isVisible = form.style.display !== 'none';
         
         // Keep enabled: English (bare fields) OR the currently visible tab
         if (languageId !== primaryLanguageId && !isVisible) {
            form.querySelectorAll('input, textarea, select').forEach(function(input) {
               input.disabled = true;
            });
         }
      ...
      // Add custom form validation for forms with data-language-required-mode="active"
      const forms = document.querySelectorAll('form[data-language-required-mode="active"]');
      forms.forEach(function(form) {
         ...
         form.addEventListener('submit', function(e) {
            ...
            if (mode === 'active') {
               // Check if any language has required fields filled
               if (!checkData()) {
                  e.preventDefault();
                  e.stopPropagation();
                  return false;
...
<script>
   ...
   $('#' + modalId + ' input[required], #' + modalId + ' textarea[required]').each(function() {
      if ($(this).val().trim() === '') {
         $(this).next('small.help-block').text('{{ __("messages.field_required") }}');
         isValid = false;
      }
   });
   if (!isValid) {
...
<script>
window.AppDataTableI18n = {
    searchLabel: @json(__('landingpage.search')),
    searchPlaceholderRaw: @json(__('messages.search')),
    displayEntries: @json(__('landingpage.display_entries')),
    showingInfo: @json(__('landingpage.showing_info')),
    emptyTable: @json(__('landingpage.no_data_available_in_table')),
    noDataFound: @json(__('messages.no_data_found')),
    previousLabel: @json(__('landingpage.previous')),
    nextLabel: @json(__('landingpage.next')),
    processingText: @json(__('messages.processing'))
};
</script>
<script src="{{ asset('js/modules/datatable-i18n.js') }}?v={{ @filemtime(public_path('js/modules/datatable-i18n.js')) }}"></script>
 @yield('bottom_script')
...
@endif
 ...
 {{-- Disable Apply button when no rows selected (all admin list pages with bulk action) --}}
 <script>
 (function() {
     function updateQuickActionApplyState() {
         var applyButtonIds = ['quick-action-apply', 'referral_quick-action-apply', 'redeem_quick-action-apply', 'earn_quick-action-apply'];
         var actionSelectIds = ['quick-action-type', 'referral_quick-action-type', 'redeem_quick-action-type', 'earn_quick-action-type'];
         for (var i = 0; i < applyButtonIds.length; i++) {
             var $apply = $('#' + applyButtonIds[i]);
             if ($apply.length === 0) continue;
             var $actionSelect = $('#' + actionSelectIds[i]);
             var actionValue = $actionSelect.length ? ($actionSelect.val() || '').trim() : '';
             var checkedCount = $('#datatable_wrapper .select-table-row:checked').length;
             var shouldDisable = (actionValue === '' || checkedCount === 0);
             $apply.prop('disabled', shouldDisable);
         }
     }
     $(document).ready(function() {
         updateQuickActionApplyState();
     });
     $(document).on('change', '#datatable_wrapper .select-table-row, #datatable_wrapper input[name="datatable_ids[]"], #datatable_wrapper #select-all-table, #datatable_wrapper [id^="select-all-table"]', function() {
         updateQuickActionApplyState();
     });
     $(document).on('change', '#quick-action-type, #referral_quick-action-type, #redeem_quick-action-type, #earn_quick-action-type', function() {
         setTimeout(updateQuickActionApplyState, 0);
     });
     $(document).on('draw.dt', '#datatable', function() {
         updateQuickActionApplyState();
     });
     window.updateQuickActionApplyState = updateQuickActionApplyState;
 })();
 </script>
