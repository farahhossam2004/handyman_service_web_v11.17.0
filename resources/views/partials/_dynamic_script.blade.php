...
(function($) {
    ...
    $(document).ready(function(){
        ...
        $(document).on('change','.change_status', function() {
            ...
            var previousState = !status; // Store the previous state (opposite of current)
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.status == false){
                        ...
                        $checkbox.prop('checked', previousState == 1 ? true : false);
                    }else{
        ...
        function ajaxMethodCall(selectDiv) {
            ...
            $.post(loadurl, function(data) {
                ...
                $(targ).find('form[data-toggle="validator"]').validator();
                // Earning setting: show Auto Free Trial only when earning type is subscription
                var $earningType = $(targ).find('#earning_type');
                if ($earningType.length) {
                    function toggleAutoFreeTrial() {
                        var earningType = $earningType.val();
                        if (earningType === 'subscription') {
                            $(targ).find('#auto-free-trial-section').slideDown();
                        } else {
                            $(targ).find('#auto-free-trial-section').slideUp();
                        }
                    }
                    toggleAutoFreeTrial();
                    $earningType.off('change.earningToggle').on('change.earningToggle', toggleAutoFreeTrial);
                }
        ...
        function confirmation(form,title = "{{ __('messages.confirmation') }}",message = "{{ __('messages.delete_msg') }}",ajaxtype=false,_this){
            ...
            buttons: {
                yes: {
                    text: '{{ __("messages.yes") }}',
                    action: function () {

                        if(ajaxtype == 'true') {
                            let url = (typeof _this === 'string') ? _this : ($(_this).attr('href') || $('[data--submit="'+form+'"]').closest('form').attr('action'));
                            ...
                            $.ajax({
                                ...
                                var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : (xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : '{{ __("messages.something_wrong") }}';
                                errorMessage(msg);
                            });
                        } else {
