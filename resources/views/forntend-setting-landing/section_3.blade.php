...
{{ html()->submit(__('messages.save'))->class('btn btn-md btn-primary float-md-end submit_section1')->id('section3_save_btn') }}
{{ html()->form()->close() }}
...
<script>
    ...
    function toggleSection3SaveBtn() {
        var enabled = $("input[name='status']").prop('checked');
        if (!enabled) {
            $('#section3_save_btn').prop('disabled', false);
            return;
        }
        ...
        $('#section3_save_btn').prop('disabled', !isValid);
    }
    ...
    $(document).ready(function() {
        ...
        if (typeof setRequiredFields === 'function') setRequiredFields(primaryLangId);
        toggleSection3SaveBtn();
        $(document).on('input', '#title_' + primaryLangId, toggleSection3SaveBtn);
        $(document).on('change', '#service_id', toggleSection3SaveBtn);
