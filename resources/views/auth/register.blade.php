                        ...
                        <div class="row">
                           ...
                           <div class="col-lg-12">
                              <div class="form-group">
                                 ...
                                 <input class="form-control" type="password" id="password" name="password" required autocomplete="new-password" placeholder="{{ __('auth.enter_name',[ 'name' => __('auth.login_password') ]) }}" minlength="8" maxlength="12">
                           ...
                           <div class="col-lg-12">
                              <div class="form-group">
                                 ...
                                 <input class="form-control" onkeyup="checkPasswordMatch()" type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password" placeholder="{{ __('auth.enter_name',[ 'name' => __('auth.confirm_password') ]) }}" minlength="8" maxlength="12">
...
@endif
                           ...
                           <div class="col-lg-12" id="provider_section" style="display: none;">
                             ...
                             <!-- Address Field for Provider -->
                             <div class="col-lg-12" id="address_section" style="display: {{ (isset($nearby_provider) && $nearby_provider == 1) ? 'block' : 'none' }};">
                                 <div class="form-group">
                                     <label for="address" class="text-secondary">{{ __('messages.address') }} @if(isset($nearby_provider) && $nearby_provider == 1) <span class="text-danger">*</span> @endif</label>
   ...
   $(document).ready(function() {
        ...
        let isContactNumberValid = true;
         ...
         function validatePasswordStrength(password) {
            if (!password || password.length < 8 || password.length > 12) return "{{ __('messages.password_length_8_12') }}";
            if (!passwordPattern.test(password)) return "{{ __('messages.password_must_contain') }}";
            return "";
         }
         function checkPasswordMatch() {
            ...
            const pwdMsg = validatePasswordStrength(password);

            if (pwdMsg) {
               passwordError.text(pwdMsg).show();
         ...
         $(document).ready(function() {
            $("#password").on("input", function() {
               var msg = validatePasswordStrength($(this).val().trim());
               if (msg) $("#password-error").text(msg).show(); else $("#password-error").hide().text("");
            });
         });
         ...
         $(document).ready(function() {
            ...
            const nearbyProviderSetting = {{ $nearby_provider ?? 0 }};
            ...
            $('#user_type').change(function() {
                ...
                // Show address and location sections for providers only if nearbyProviderSetting is enabled
                $('#address_section').toggle(selectedUserType === 'provider' && nearbyProviderSetting == 1);
...
// });
        ...
        $('form').on('submit', function(e) {
            ...
            var pwdMsg = typeof validatePasswordStrength !== 'undefined' ? validatePasswordStrength(pwd) : '';
            if (pwdMsg) {
                e.preventDefault();
                $('#password-error').text(pwdMsg).show();
                return false;
            }
            ...
            // Get number in E.164 format, e.g. +919876543210
            let cleanNumber = iti.getNumber();

            // Store in E.164 format (same as PhoneInputHandler)
            $('#phone_number').val(cleanNumber);
            $('#phone_number_err').text('');
        });
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Listen for file input changes inside the provider document section
            document.querySelectorAll('#provider_document_section input[type="file"]').forEach(function (input) {
                input.addEventListener('change', function (e) {
                    const file = e.target.files[0];
                    const errorElement = input.parentElement.querySelector('.help-block');
                    ...
                    if (file) {
                        if (file.size > maxFileSize) {
                            ...
                            errorElement.textContent = `File size must be less than ${maxMB} MB.`;
