        ...
        <div class="row">
            <div class="col-md-12">
                <div class="row">
                     <!-- Downgrade Banner -->
                    @php
                        $downgrade_banner = get_provider_downgrade_banner(auth()->user()->id);
                        $upgrade_banner = get_provider_upgrade_banner(auth()->user()->id);
                        // Check if provider has at least one active service or handyman
                        $has_active_resources = \App\Models\Service::where('provider_id', auth()->user()->id)
                            ->where('status', 1)
                            ->exists() || 
                            \App\Models\User::where('provider_id', auth()->user()->id)
                            ->where('status', 1)
                            ->exists();
                    @endphp
                    @if(!empty($downgrade_banner) && !$has_active_resources)
                        <div class="col-md-12">
                            <div class="alert alert-warning border border-warning py-3" role="alert">
                                <div class="d-flex align-items-start">
                                    <div class="me-1">
                                        <i class="fas fa-info-circle"></i>
                                    </div>
                                    <div class="flex-grow-1">
                                        <h5 class="alert-heading mb-2">{{ $downgrade_banner['title'] }}</h5>
                                        <p class="mb-0">{{ $downgrade_banner['description'] }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if(!empty($upgrade_banner) && !$has_active_resources ) 
                        <div class="col-md-12">
                            <div class="alert alert-warning border border-warning py-3" role="alert">
                                <div class="d-flex align-items-start">
                                    <div class="me-1">
                                    ...
                                    <div class="flex-grow-1">
                                        <p class="mb-0">{{ $upgrade_banner['description'] }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
