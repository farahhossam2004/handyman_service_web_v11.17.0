    ...
    <div class="container-fluid">
        ...
        <div class="col-lg-12">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        {{ html()->form('POST', route('handymantype.store'))->attribute('data-toggle', 'validator')->attribute('data-language-required-mode', 'active')->id('handymantype')->open() }}
                            ...
                            @foreach(['name' => __('messages.name')] as $field => $label)
                                <div class="form-group col-md-{{ $field === 'name' ? '4' : '12' }}">
                                    ...
                                    @php
                                        ...
                                        if ($handymantypedata && $handymantypedata->id) {
                                            $translation = $handymantypedata->translations()
                                        ...
                                        // - English tab → always show main column (main col stores English)
                                        // - Any other language → show translation row only, empty if none exists
                                        if ($language['id'] === 'en') {
                                            // Main column always stores English
                                            $value = $handymantypedata->$field ?? '';
                                        } else {
                                            // Non-English: only show if translation row exists, otherwise empty
                                            $value = ($translation !== null && $translation !== '') ? $translation : '';
                                        }
                                        
                                        // Field name: English always posts as bare field (stored in main column)
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                ...
                                {{ html()->number('commission', $handymantypedata->commission )->attributes(['min' => 0,'step' => 'any'])->placeholder(__('messages.commission'))->class('form-control')->id('commission') }}
                            </div>
