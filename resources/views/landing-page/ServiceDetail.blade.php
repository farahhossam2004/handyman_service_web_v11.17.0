            ...
            <div class="row">
                <div class="col-lg-8 pe-xxl-5">
                                ...
                                <div
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input service-addon-checkbox" type="checkbox"
                                            ...
                                            data-addon-price="{{ $serviceaddon['price'] }}" data-addon-name="{{ $serviceaddon['name'] }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                                ...
                                <div class="mt-5 border-top">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="pe-0 py-2 text-end">
                                                        ...
                                                        <h6 class="text-primary" id="subtotal-amount">{{ getPriceFormat($subtotal) }}</h6>
                                                    </td>
                                                </tr>
                                                <tr id="addons-row" style="display: none;">
                                                    <td class="ps-0 py-2">
                                                        <h6 class="text-capitalize m-0">Service {{ __('landingpage.Add-ons') }}</h6>
                                                    </td>
                                                    <td class="pe-0 py-2 text-end">
                                                        <h6 class="text-primary" id="addons-amount">+{{ getPriceFormat(0) }}</h6>
                                                    </td>
                                                </tr>
                                                @php
                                                    ...
                                                    <td class="pe-0 py-2 text-end">
                                                        @if ($totalTaxAmount > 0)
                                                            <span class="text-danger"><i type="button"
                                                                ...
                                                                <span id="tax-amount-value">+{{ getPriceFormat($totalTaxAmount) }}</span>
                                                            </span>
                                                        @else
                                                            <span class="text-danger">
                                                                <span id="tax-amount-value"> +{{ getPriceFormat(0) }}</span>
                                                            </span>
                                                        @endif
                                                    </td>
                                                </tr>
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title text-capitalize"
                                                                    id="taxModalLabel">{{ __('messages.applied_taxes') }}
                                                                </h5>
                                                                <span class="text-primary custom-btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="40"
                                                                        height="41" viewBox="0 0 40 41"
                                                                        fill="none">
                                    ...
                                    <div class="text-end">
                                        @if (auth()->check() && auth()->user()->user_type == 'user')
                                            <a href="{{ route('book.service', ['id' => $serviceData['service_detail']['id'], 'shop_id' => $shop_id ?? null]) }}"
                                                class="btn btn-lg btn-primary continue-button">{{ __('messages.continue') }}
                                                (<span class="continue-amount">{{ getPriceFormat($total) }}</span>)</a>
                                        @else
                                            <a href="{{ route('user.login', ['service_id' => $serviceData['service_detail']['id'], 'shop_id' => $shop_id ?? null]) }}"
                                                class="btn btn-lg btn-primary continue-button">{{ __('messages.continue') }}
                                                (<span class="continue-amount">{{ getPriceFormat($total) }}</span>)</a>
                                        @endif
                                    </div>
                    ...
                    @if ($total_ratings->isNotEmpty())
                        <div class="section-padding px-0 pb-5">
                    ...
                    @if (!empty($shops) && count($shops) > 0)
                        <div class="row align-items-center pt-3 mt-5 mb-5">
                            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap mt-3">
                                <h5 class="text-capitalize">{{ __('messages.shops') }}</h5>
                                <a
                                    href="{{ route('shop.list', ['service_id' => $serviceData['service_detail']['id']]) }}">{{ __('messages.view_all') }}</a>
                            </div>
                            <div
                                class="swiper-container swiper-container-initialized swiper-container-horizontal swiper-container-pointer-events shop-swiper-container">
                                                    ...
                                                    <div class="service-box-card bg-light rounded-3 mb-5">
                                                        ...
                                                        <a href="/" class="service-heading mt-4 d-block p-0">
                                                            <h5 class="service-title font-size-18 line-count-2"><a
                                                                    href="{{ route('shop.detail', $shop->id) }}">{{ $shop->translate('shop_name') }}</a>
                                                            </h5>
                                                        </a>
                                                        <div class="mt-3">
                                                            <div class="d-flex align-items-center gap-2">
                                                                ...
                                                                <a
                                                                    href="{{ route('provider.detail', $shop->provider->id) }}"><span
                                                            ...
                                                            <div class="small text-muted">
                                                                <div class="d-flex align-items-center mb-2">
                                                                    ...
                                                                    @php
                                                                        ...
                                                                        $targetTz = $siteSetup->time_zone ?? 'UTC'; // e.g. Asia/Kolkata
                                                                        $timeFmt = $siteSetup->time_format ?? 'H:i'; // e.g. g:i A
                                                                    @endphp

                                                                    {{ \Carbon\Carbon::parse($shop->getRawOriginal('shop_start_time'), 'UTC')->setTimezone($targetTz)->format($timeFmt) }}
                                                                    -
                                                                    {{ \Carbon\Carbon::parse($shop->getRawOriginal('shop_end_time'), 'UTC')->setTimezone($targetTz)->format($timeFmt) }}
                                                                </div>
                    ...
                    <div class="mt-5 pt-lg-5 pt-3">
                        ...
                        <div class="position-relative overflow-hidden swiper swiper-general our-service" data-slide="2"
                            ...
                            @php
                                ...
                                $filteredRelatedServices = array_filter($serviceData['related_service'], function (
                                    $service,
                                ) use ($currentServiceId) {
                            ...
                            <landing-servicedetailsection-section
                                ...
                                :user_id="{{ $userId }}" :favourite="{{ json_encode($favouriteServiceData) }}">
                            </landing-servicedetailsection-section>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex-shrink-0">
                                @if (auth()->check() && auth()->user()->hasRole('user'))
                                    @if (empty($favouriteService))
                                        <form method="POST" id="favoriteForm">
                                            ...
                                            <button type="button"
                                                class="btn btn-light bg-white rounded-circle serv-whishlist text-primary p-0 avatar-30 lh-normal save_fav">
                                    ...
                                    @else
                                        <form method="POST" id="favoriteForm">
                                            ...
                                            @endif
                                            <button type="button"
                                                class="btn btn-light bg-white rounded-circle serv-whishlist text-primary p-0 avatar-30 delete_fav lh-normal">
                                ...
                                @else
                                    <form method="GET" id="favoriteForm" action="{{ route('user.login') }}">
                                        @csrf
                                        <button type="submit"
                                            class="btn btn-light bg-white rounded-circle serv-whishlist text-primary p-0 avatar-30 btn btn-light bg-white rounded-circle serv-whishlist text-primary p-0 avatar-30 delete_fav lh-normal">
                                            <svg width="16" height="16" viewBox="0 0 12 13" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" clip-rule="evenodd"
                                                    d="M1.43593 6.29916C0.899433 4.62416 1.52643 2.70966 3.28493 2.14316C4.20993 1.84466 5.23093 2.02066 5.99993 2.59916C6.72743 2.03666 7.78593 1.84666 8.70993 2.14316C10.4684 2.70966 11.0994 4.62416 10.5634 6.29916C9.72843 8.95416 5.99993 10.9992 5.99993 10.9992C5.99993 10.9992 2.29893 8.98516 1.43593 6.29916Z"
                                ...
                                <ul class="list-inline m-0 p-0 d-flex align-items-center gap-2 flex-wrap">
                                    ...
                                    @foreach ($serviceData['service_detail']['service_zone_mappings'] as $zoneMapping)
                                        @if (!empty($zoneMapping['zone']['name']))
                                            ...
                                            <li>
                                                <span
                                                    class="btn btn-sm btn-outline-primary text-capitalize cursor-default d-flex align-items-center gap-2">
...
@section('after_script')
    <script>
        $(document).ready(function() {
            const baseSubtotal = Number(@json($subtotal ?? 0));
            const serviceTaxes = @json($serviceData['taxes'] ?? []);

            function formatMoney(value) {
                const num = Number(value) || 0;
                if (window.currencyFormat !== undefined) {
                    return window.currencyFormat(num);
                }
                return num.toFixed(2);
            }

            function recalcOrderDetailWithAddons() {
                let addonTotal = 0;
                $('.service-addon-checkbox:checked').each(function() {
                    addonTotal += Number($(this).data('addon-price')) || 0;
                });

                const newSubtotal = baseSubtotal + addonTotal;

                // Tax calculation like backend:
                // percent => (subtotal * value)/100, fixed => value
                let taxAmount = 0;
                if (Array.isArray(serviceTaxes) && serviceTaxes.length) {
                    serviceTaxes.forEach(function(tax) {
                        const value = Number(tax.value) || 0;
                        if (tax.type === 'percent') {
                            taxAmount += (newSubtotal * value) / 100;
                        } else {
                            taxAmount += value;
                        }
                    });
                ...
                $('#subtotal-amount').text(formatMoney(newSubtotal));

                if (addonTotal > 0) {
                    $('#addons-row').show();
                    $('#addons-amount').text('+' + formatMoney(addonTotal));
                } else {
                    $('#addons-row').hide();
                }

                $('#tax-amount-value').text((taxAmount > 0 ? '+' : ' ') + formatMoney(taxAmount));

                $('.continue-button .continue-amount').text(formatMoney(newTotal));
            }
            ...
            function updateContinueButtonHref() {
                ...
                var baseUrl =
                    `{{ route('book.service', ['id' => $serviceData['service_detail']['id'], 'shop_id' => $shop_id ?? null]) }}`;

                var updatedHref = baseUrl + '&addons=' + selectedAddonIds.join(',');

                $('.continue-button').attr('href', updatedHref);

                // Update amounts shown in Order Detail + Continue button
                recalcOrderDetailWithAddons();
            }
            // Initial sync (in case any addon is pre-checked)
            recalcOrderDetailWithAddons();
        ...
        document.addEventListener("DOMContentLoaded", function() {
            const swiper = new Swiper('.shop-swiper-container', {
                loop: false,
                speed: 600,
                spaceBetween: 30,
                slidesPerView: 3,
