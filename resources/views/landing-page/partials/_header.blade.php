                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                   <ul class="navbar-nav align-items-center ms-auto mb-2 mb-xl-0 p-0">
                     ...
                     @endif
                      ...
                      <!-- Wishlist -->
                      @php
                          $__nbSetting = \App\Models\Setting::where('type','OTHER_SETTING')->where('key','OTHER_SETTING')->first();
                          $__nbEnabled = $__nbSetting && !empty(json_decode($__nbSetting->value)->nearby_provider);
                      @endphp
                      @if($__nbEnabled)
                      <li class="nav-item ms-sm-2 ms-1">
                          <a id="nav-nearby-btn"
                              href="{{ route('frontend.nearby_providers') }}"
                              class="btn btn-primary d-inline-flex align-items-center gap-1"
                              style="white-space:nowrap; padding: 6px 14px; font-size: 14px;">
                              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5s2.5 1.12 2.5 2.5S13.38 11.5 12 11.5z"/>
                              </svg>
                              {{ __('messages.browse_nearby') ?? 'Browse Nearby' }}
                          </a>
                      </li>
                      @endif
                      @if($__nbEnabled)
                      ...
                      document.addEventListener('DOMContentLoaded', function(){
                          var btn = document.getElementById('nav-nearby-btn');
                          if (!btn || !navigator.geolocation) return;
                          var baseUrl = '{{ route("frontend.nearby_providers") }}';
                          btn.addEventListener('click', function(e){
                              e.preventDefault();
                              navigator.geolocation.getCurrentPosition(
                                  function(pos){
                                      window.location.href = baseUrl + '?latitude=' + pos.coords.latitude + '&longitude=' + pos.coords.longitude;
                                  },
