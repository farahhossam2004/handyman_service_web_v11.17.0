                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            ...
                            <h5 class="mt-2 mb-2 text-capitalize">#{{ $bookingData['booking_detail']['id'] }}
                                {{ $bookingData['booking_detail']['booking_package']['name'] }}
                            </h5>
                            @endif
                            ...
                            <h6 class="m-0 text-capitalize text-body">{{ __('landingpage.service_total_time') }}:
                                <span class="text-primary">
                                    <span id="live-timer" class="d-inline-block fw-500 position-relative service-price">
                                        @php
                                        $durationParts = $bookingData['booking_detail']['duration_diff_hour'] ?? '00:00:00';
                                        @endphp
                                        {{ $durationParts }}
                                    </span>
                                </span>
                    ...
                    <div class="col-12 col-sm-6">
                        <div class="bg-light p-3 rounded h-100">
                            <p class="mb-0"><i
                                ...
                                {{ $bookingData['shop_data']['country_name'] }}
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table table-borderless mb-0">
                                <tbody>
                                    ...
                                    $bookingData['booking_detail']['final_coupon_discount_amount'] < $bookingData['booking_detail']['total_amount']
                                        ...
                                        @if (
                                        !empty($bookingData['booking_detail']['redeemed_discount']) &&
                                        !empty($bookingData['booking_detail']['redeemed_points']))
                                        <tr>
                                            <td>
                                                <span class="text-capitalize">{{ __('messages.redeem_discount') }}</span><span class="text-primary"> ( {{ $bookingData['booking_detail']['redeemed_points'] }} pts )</span>
                                            </td>
                                            <td>
                                                <span class="d-block text-end text-success">
                                                    -{{ getPriceFormat($bookingData['booking_detail']['redeemed_discount']) }}
                                                </span>
                                            </td>
                                        </tr>
                                        @endif
                ...
                <div class="pt-lg-5 pt-3 mt-lg-5 mt-3">
                    <div class="mt-3">
                        <h5 class="mb-5">{{ count($bookingData['rating_data']) }}
                            ...
                            {{ $bookingData['booking_detail']['service_name'] }}
                        </h5>
        ...
        <div class="modal-content overflow-visible">
            <div class="modal-header border-bottom-0 gap-2">
                <h5 class="modal-title text-capitalize" id="statusModalLabel">
                    {{ __('landingpage.booking_history') }}
                </h5>
                <span class="text-primary custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41"
                        fill="none">
                        ...
                        <div class="status-timeline-wrapper position-relative">
                            <div class="timeline-time-block text-md-end">
                                <h6 class="text-uppercase m-0 time">
                                    {{ date($date_time['time_format'], strtotime($time)) }}
                                </h6>
                                <p class="text-capitalize m-0 date">
                                    {{ date($date_time['date_format'], strtotime($date)) }}
                                </p>
                            </div>
                            <div class="timeline-content-block text-md-start">
                                ...
                                <p class="text-capitalize m-0 description">
                                    {{ $bookingactivity['activity_message'] }}
                                </p>
                            </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title text-capitalize fw-bold" id="ratingModalLabel">
                    {{ __('landingpage.reason_modal_msg') }}
                </h6>
                <span class="text-primary custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41"
                        fill="none">
...
<div class="modal fade" id="cancelModal" aria-labelledby="reasonModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title text-capitalize fw-bold" id="ratingModalLabel">
                    {{ __('landingpage.cancel_modal_msg') }}
                </h6>
                <span class="text-primary custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41"
                        fill="none">
...
<script>
    ...
    let previousDurationSeconds = {{ $bookingData['booking_detail']['duration_diff'] ?? 0 }}; // Now in seconds
    
    // Initialize timer based on booking status
    const bookingStatus = '{{ $bookingData['booking_detail']['status'] }}';
    const startAt = '{{ $bookingData['booking_detail']['start_at'] ?? '' }}';
    ...
    function startLiveTimer() {
        ...
        if (startAt) {
            ...
            elapsedSeconds = Math.floor(elapsedMs / 1000) + previousDurationSeconds; // Both in seconds now
        } else {
            elapsedSeconds = previousDurationSeconds;
        }
    ...
    if (bookingStatus === 'in_progress' && startAt) {
        startLiveTimer();
    }
    
    // Stop timer when page is hidden/closed
    document.addEventListener('visibilitychange', function() {
        if (document.hidden && bookingStatus === 'in_progress') {
            // Keep timer running even when tab is hidden
        }
    });
    ...
    function startBooking() {
        ...
        // Don't start timer yet - wait for page reload with updated data
    ...
    function holdBooking() {
        ...
        const currentSessionSeconds = Math.floor((new Date(end_at) - new Date(start_at)) / 1000);
        
        // Get previous duration in seconds from DB
        const previousDurationSeconds = parseInt(document.getElementById('duration_diff').value) || 0;
        
        // Total duration in seconds
        duration_diff = previousDurationSeconds + currentSessionSeconds;
    ...
    function resumeBooking() {
        ...
        previousDurationMinutes = parseInt(duration_diff);
