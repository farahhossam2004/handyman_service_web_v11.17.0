...
const getResolvedLocation = () => {
  const propLat = props.user_lat;
  const propLng = props.user_lng;
  if (propLat != null && propLng != null && propLat !== '' && propLng !== '') {
    return { lat: Number(propLat), lng: Number(propLng) };
  }

  const storageLat = localStorage.getItem('loction_current_lat');
  const storageLng = localStorage.getItem('loction_current_long');
  if (storageLat && storageLng) {
    return { lat: Number(storageLat), lng: Number(storageLng) };
  }
...
const fetchService = async () => {
  try {
    ...
    const isLocationOn = localStorage.getItem('is_location_on') === 'on';
    const hasLocation = !!resolvedLocation && isLocationOn;
    
    const serviceQuery = {
      per_page: 'all',
      status: 1,
      request_status: 'approve',
      location_enabled: hasLocation ? 1 : 0,
    };

    if (hasLocation) {
      serviceQuery.latitude = resolvedLocation.lat;
      serviceQuery.longitude = resolvedLocation.lng;
    }

    const response = await fetch(SERVICE_API(serviceQuery));
  ...
  try {
    ...
    if (settings) {
      ...
      if (props.type === 'recently_view') {
        ...
        await fetchService();
        // Use frontend settings (section_3 or section_4) for service IDs
      ...
      if (hasLocation) {
        // If location is ON, filter zone-filtered services by selected service IDs from frontend settings
        selectedServices = allService.filter(service => serviceIds.includes(String(service.id)));
      } else {
        // If location is OFF, use hardcoded service IDs from frontend settings
        selectedServices = allService.filter(service => serviceIds.includes(String(service.id)));
      }
        ...
        const filteredServices = selectedServices.filter(service => {
          if (providers.length > 0) {
            ...
            if (provider && provider.id && provider.status === 1 && provider.is_subscribe === 1) {
              ...
              console.log(`[${props.type}] Service filtered out:`, {
                serviceId: service.id,
                serviceName: service.name,
                providerId: service.provider_id,
                providerFound: !!provider,
                providerStatus: provider?.status,
                providerSubscribed: provider?.is_subscribe
              });
            }
          }
          return false;
