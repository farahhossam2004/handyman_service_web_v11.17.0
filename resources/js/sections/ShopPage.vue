<template>
    <section ref="providersection">
        <div class="row align-items-center">
                ...
                <div class="row gx-3">
                    <div class="col-md-4">
                        <select ref="providerDropdown" class="form-select select2" name="provider" id="provider" v-model="provider">
                            <option value="">{{$t('landingpage.all_providers')}}</option>
                            <option v-for="providers in provider_data" :key="providers.id" :value="providers.id">{{ providers.first_name }} {{ providers.last_name }}</option>
                        </select>
                    </div>
                </div>
            </div>
...
const provider = ref('')
watch(() => provider.value, () => ajaxReload())
...
useDataTable({
  ...
  advanceFilter: () => {
    const locale = (typeof window !== 'undefined' && window.frontendLocale) || sessionStorage.getItem('local') || 'en';
    return {
        search: search.value,
...
onMounted(() => {
    $(providerDropdown.value).select2({
        width: '100%'
    });
    $(providerDropdown.value).on('change', function() {
        provider.value = $(this).val();
    });
    loadProviderData();
});
