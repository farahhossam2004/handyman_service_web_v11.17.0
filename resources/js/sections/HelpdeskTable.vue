        ...
        <div class="row">
            <div class="col-md-6 col-sm-8">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <a class="btn-tab-desk" :class="{ active: filterStatus === '0' }" href="javascript:void(0);"
                        @click="setFilterStatus('0')">{{ $t('messages.open') }}</a>
                    <a class="btn-tab-desk" :class="{ active: filterStatus === '1' }" href="javascript:void(0);"
                        @click="setFilterStatus('1')">{{ $t('messages.closed') }}</a>
                </div>
            </div>
...
// Ensure DataTables is available globally
...
// Helper function to check if DataTable is ready
const isDataTableReady = () => {
    return tableRef.value && $ && $.fn && $.fn.DataTable && $.fn.DataTable.isDataTable(tableRef.value);
};

const setFilterStatus = (value) => {
    filterStatus.value = value;
    status.value = value;
    // Add longer delay to ensure DataTable is fully ready
    setTimeout(() => {
        ajaxReload();
    }, 300);
};
...
// Datatable
const ajaxReload = () => {
    const attemptReload = (retries = 3) => {
        try {
            if (isDataTableReady()) {
                $(tableRef.value).DataTable().ajax.reload(null, false);
            } else if (retries > 0) {
                setTimeout(() => attemptReload(retries - 1), 200);
            }
        } catch (error) {
            if (retries > 0) {
                setTimeout(() => attemptReload(retries - 1), 200);
...
onMounted(() => {
    defaultData()
    // Wait longer for DOM and DataTable to be ready before initializing filter
    setTimeout(() => {
        if (isDataTableReady()) {
            setFilterStatus('');
        } else {
            // If DataTable is not ready, wait a bit more
            setTimeout(() => {
                setFilterStatus('');
            }, 500);
        }
