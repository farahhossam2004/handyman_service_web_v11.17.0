...
trait NotificationTrait
{
    function sendNotification($data): void
    {
        ...
        switch ($data['activity_type']) {
            case "add_booking":
                ...
                // Add activity_slug for add_booking
                $data['activity_slug'] = 'add_booking';
                $activity_data = [
            ...
            case "add_helpdesk":
                ...
                // Add activity_slug for add_helpdesk
                $data['activity_slug'] = 'add_helpdesk';
            ...
            case "closed_helpdesk":
                ...
                // Add activity_slug for closed_helpdesk
                $data['activity_slug'] = 'closed_helpdesk';
                $data['helpdesk_id'] = $helpdesk->id;
            ...
            case "reply_helpdesk":
                ...
                // Add activity_slug for reply_helpdesk
                $data['activity_slug'] = 'reply_helpdesk';
                $data['helpdesk_id'] = $helpdesk->id;
                $data['sender_id'] = is_array($data['sender_id']) ? $data['sender_id'][0] : $data['sender_id'];
                ;
            ...
            case "assigned_booking":
                ...
                // Add activity_slug for assigned_booking
                $data['activity_slug'] = 'assigned_booking';
            ...
            case "transfer_booking":
                ...
                // Add activity_slug for transfer_booking
                $data['activity_slug'] = 'transfer_booking';
            ...
            case "update_booking_status":
                ...
                // Add activity_slug for update_booking_status
                $data['activity_slug'] = 'update_booking_status';
            ...
            case "cancel_booking":
                ...
                $data['activity_type'] = __('messages.booking_cancelled');
                // Add activity_slug for cancel_booking
                $data['activity_slug'] = 'cancel_booking';
                $data['activity_message'] = __('messages.booking_cancelled');
                $handymanId = $booking->handymanAdded ? $booking->handymanAdded->pluck('handyman_id') : null;
                $activity_data = [
                    'reason' => $booking->reason,
                    'status' => $booking->status,
            ...
            case "payment_message_status":
                ...
                // // Add activity_slug for payment_message_status
                // $data['activity_slug'] = 'payment_message_status';
                // $data['activity_message'] = __('messages.payment_message', ['status' => $data['payment_status']]);

                if ($data['payment_status'] === 'advanced_paid') {
                    $data['activity_type'] = __('messages.advanced_payment_message_status');
                    $data['activity_message'] = __('messages.advance_payment_message', ['amount' => isset($data['pay_amount']) ? getPriceFormat($data['pay_amount']) : '']);
                    $data['activity_slug'] = 'advanced_paid';
            ...
            case "wallet_payout_transfer":
                ...
                $provider = \App\Models\User::find($data['provider_id']);
                $data['user_id'] = !empty($data['debit_user']) ? $data['debit_user']: $handyman_id ?? null;
                $handyman = \App\Models\User::find($handyman_id);
                $data['activity_message'] = !empty($data['debit_user']) ? __('messages.you_payout_paid_handyman', ['amount' => getPriceFormat($data['transfer_amount']), 'user' => isset($handyman->display_name) ? $handyman->display_name : 'unknown']) : __('messages.provider_payout_paid_handyman', ['provider' =>  isset($provider->display_name) ? $provider->display_name : 'unknown','amount' => getPriceFormat($data['transfer_amount']), 'user' => isset($handyman->display_name) ? $handyman->display_name : 'unknown']);
                ...
                // Add activity_slug for wallet_payout_transfer
                $data['activity_slug'] = 'wallet_payout_transfer';
                $activity_data = [
            ...
            case "wallet_top_up":
                ...
                // Add activity_slug for wallet_top_up
                $data['activity_slug'] = 'wallet_top_up';
                $activity_data = [
                    'title' => $data['wallet']->title,
                    'user_id' => $data['wallet']->user_id,
            ...
            case "wallet_refund":
                ...
                $data['user_id'] = $data['wallet']->user_id ?? null;
                // Add activity_slug for wallet_refund
                $data['activity_slug'] = 'wallet_refund';
                $activity_data = [
                    'title' => $data['wallet']->title,
                    'user_id' => $data['wallet']->user_id,
                    'amount' => $data['wallet']->amount,
                ...
                $booking_activity = [
                    'booking_id'      => $data['booking_id'],
                    'datetime'        => now(),
                    'activity_type'   => __('messages.wallet_refunded'),
                    'activity_message'=> $data['activity_message'],
                    'activity_slug'   => 'wallet_refunded', // Add activity_slug for wallet_refunded
                ];

                $booking_activity['activity_data'] = $data['activity_data'] ;
                \App\Models\BookingActivity::create($booking_activity);
            ...
            case "cancellation_charges":
                ...
                $data['user_id'] = $data['wallet']->user_id ?? null;
                // Add activity_slug for cancellation_charges
                $data['activity_slug'] = 'cancellation_charges';
                $activity_data = [
                    'title' => $data['wallet']->title,
                    'user_id' => $data['wallet']->user_id,
                    'amount' => $data['wallet']->amount,
            ...
            case "paid_with_wallet":
                ...
                // Add activity_slug for paid_with_wallet
                $data['activity_slug'] = 'paid_with_wallet';
                $activity_data = [
                    'title' => $data['wallet']->title,
                    'user_id' => $data['wallet']->user_id,
                    'amount' => $data['wallet']->amount,
            ...
            case "job_requested":
                ...
                // Add activity_slug for job_requested
                $data['activity_slug'] = 'job_requested';
                $customerLatitude = $data['latitude'] ;
                $customerLongitude = $data['longitude'] ;
                $radius = getSettingKeyValue('site-setup', 'radious') ?? 50;
            ...
            case "user_accept_bid":
                ...
                // Add activity_slug for user_accept_bid
                $data['activity_slug'] = 'user_accept_bid';
                $data['provider_name'] = $post_job->provider->display_name ?? null;
            ...
            case "provider_send_bid":
                ...
                // Add activity_slug for provider_send_bid
                $data['activity_slug'] = 'provider_send_bid';
            ...
            case "provider_payout":
                ...
                // Add activity_slug for provider_payout
                $data['activity_slug'] = 'provider_payout';
                $activity_data = [
            ...
            case "handyman_payout":
                ...
                $data['pay_amount'] = isset($data['amount']) ? $data['amount'] : '';
                $data['payout_date'] = isset($data['pay_date']) ? $data['pay_date'] : '';
                // Add activity_slug for handyman_payout
                $data['activity_slug'] = 'handyman_payout';
                $activity_data = [
                    'user_id' => $data['user_id'],
                    'amount' => $data['amount'],
                    'credit_debit_amount' => $data['amount'],
            ...
            case "subscription_add":
                ...
                // Add activity_slug for subscription_add
                $data['activity_slug'] = 'subscription_add';
                $activity_data = [
            ...
            case "subscription_cancelled":
                $id = $data['user_id'];
                $providerId = [$data['user_id']];
                $data['activity_message'] = __('messages.subscription_cancelled');
                $data['activity_slug'] = 'subscription_cancelled';
                $data['provider_name'] = $data['provider_name'] ?? '';
                $data['plan_name'] = $data['plan_name'] ?? '';
                $data['cancellation_date'] = $data['cancellation_date'] ?? '';
                break;
            case "subscription_expired":
                $id = $data['user_id'];
                $providerId = [$data['user_id']];
                $data['activity_message'] = __('messages.subscription_expired');
                $data['activity_slug'] = 'subscription_expired';
                $data['provider_name'] = $data['provider_name'] ?? '';
                $data['plan_name'] = $data['plan_name'] ?? '';
            ...
            case "email_verification":
                $id = $data['user_id'];
                $data['activity_message'] = __('messages.email_verification_sent');
                $data['activity_slug'] = 'email_verification';
                $verificationUserId = $data['user_id'] ?? null;
                break;

            case "new_shop_created":
                $id = $data['shop_id'] ?? null;
                $shop = null;
                if ($id) {
                    $shop = \App\Models\Shop::with(['city', 'state', 'country'])->find($id);
                }
                $data['activity_message'] = __('messages.new_shop_created', ['name' => $data['shop_name'] ?? '']);
                $data['activity_slug'] = 'new_shop_created';
                $data['userIds'] = User::where('user_type', 'user')->pluck('id')->toArray();
                if (!empty($data['created_by']) && $data['created_by'] === 'admin' && !empty($data['provider_id'])) {
                    $providerId = [$data['provider_id']];
                    $data['notify_to'] = ['provider', 'user'];
                } else {
                    $data['notify_to'] = ['admin', 'user'];
                }
                // Add shop details for email template
                if ($shop) {
                    $data['shop_address'] = $shop->address ?? '';
                    $data['shop_registration'] = $shop->registration_number ?? '';
                    $data['shop_city'] = $shop->city ? $shop->city->name : '';
                    $data['shop_state'] = $shop->state ? $shop->state->name : '';
                    $data['shop_country'] = $shop->country ? $shop->country->name : '';
                    $data['shop_contact'] = $shop->contact_number ?? '';
            ...
            case "shop_activated":
                ...
                $data['activity_message'] = __('messages.shop_activated', ['name' => $data['shop_name'] ?? '']);
                $data['activity_slug'] = 'shop_activated';
                if (!empty($data['changed_by']) && $data['changed_by'] === 'admin' && !empty($data['provider_id'])) {
                    $providerId = [$data['provider_id']];
                }
                if ($shop) {
                    $data['shop_address'] = $shop->address ?? '';
             ...
             case "shop_deactivated":
                ...
                $data['activity_message'] = __('messages.shop_deactivated', ['name' => $data['shop_name'] ?? '']);
                $data['activity_slug'] = 'shop_deactivated';
                if (!empty($data['changed_by']) && $data['changed_by'] === 'admin' && !empty($data['provider_id'])) {
                    $providerId = [$data['provider_id']];
                }
                if ($shop) {
            ...
            case "provider_free_plan_assigned":
                $id = $data['user_id'] ?? null;
                $data['activity_message'] = __('messages.provider_free_plan_assigned', ['name' => $data['provider_name'] ?? '']);
                $data['activity_slug'] = 'provider_free_plan_assigned';
                $providerId = [$data['user_id']];
                // Set required template variables for the email
                $data['plan_title'] = $data['plan_title'] ?? '';
                $data['plan_duration'] = $data['plan_duration'] ?? '';
                $data['plan_type'] = $data['plan_type'] ?? '';
                $data['plan_description'] = $data['plan_description'] ?? '';
                break;

            case "free_plan_expiry_reminder":
                $id = $data['user_id'] ?? null;
                $data['activity_message'] = __('messages.free_plan_expiry_reminder', ['name' => $data['provider_name'] ?? '']);
                $data['activity_slug'] = 'free_plan_expiry_reminder';
                
                // Set all required template variables for the email
                $data['provider_name'] = $data['provider_name'] ?? '';
                $data['plan_title'] = $data['plan_title'] ?? '';
                $data['expiry_date'] = $data['expiry_date'] ?? '';
                $data['plan_duration'] = $data['plan_duration'] ?? '';
            ...
            case "register":
                ...
                // Add activity_slug for register
                $data['activity_slug'] = 'register';
                $activity_data = [
                    'user_id' => $data['user_id'],
            ...
            case "withdraw_money";
                ...
                // Add activity_slug for withdraw_money
                $data['activity_slug'] = 'withdraw_money';
            ...
            case "promotional_banner":
                ...
                // Add activity_slug for promotional_banner
                $data['activity_slug'] = 'promotional_banner';
                $activity_data = [
                    'banner_id' => $id,
            ...
            case "promotional_banner_accepted":
                ...
                // Add activity_slug for promotional_banner_accepted
                $data['activity_slug'] = 'promotional_banner_accepted';
                $activity_data = [
                    'banner_id' => $id,
                    'banner_title' => $data['banner_title'],
                    'provider_id' => $data['provider_id'] ?? null,
            ...
            case "promotional_banner_rejected":
                ...
                // Add activity_slug for promotional_banner_rejected
                $data['activity_slug'] = 'promotional_banner_rejected';
                $activity_data = [
                    'banner_id' => $id ?? null,
            ...
            case "wallet_refund_promotional_banner":
                ...
                // Add activity_slug for wallet_refund_promotional_banner
                $data['activity_slug'] = 'wallet_refund_promotional_banner';
            ...
            case 'service_request':
                ...
                // Add activity_slug for service_request
                $data['activity_slug'] = 'service_request';
            ...
            case 'service_request_approved':
                ...
                $data['id'] = $data['service_id'] ?? null;
                // Add activity_slug for service_request_approved
                $data['activity_slug'] = 'service_request_approved';
                $data['data'] = [
                    'service_id' => $data['service_id'] ?? null,
                    'provider_id' => $data['provider_id'] ?? null,
                    'status' => 'approved',
            ...
            case 'service_request_reject':
                ...
                $data['id'] = $data['service_id'] ?? null;
                // Add activity_slug for service_request_reject
                $data['activity_slug'] = 'service_request_reject';
                $data['data'] = [
                    'service_id' => $data['service_id'] ?? null,
                    'provider_id' => $data['provider_id'] ?? null,
                    'status' => 'rejected',
        ...
        if (isset($booking)) {
            ...
            $notification_data['company_name'] = 'Sanad';
            ...
            $notification_data['plan_title'] = isset($data['plan_title']) ? $data['plan_title'] : (isset($data['plan_name']) ? $data['plan_name'] : '');
            $notification_data['plan_duration'] = isset($data['plan_duration']) ? $data['plan_duration'] : '';
            $notification_data['plan_type'] = isset($data['plan_type']) ? $data['plan_type'] : '';
            $notification_data['plan_description'] = isset($data['plan_description']) ? $data['plan_description'] : '';
            $notification_data['expiry_date'] = isset($data['expiry_date']) ? $data['expiry_date'] : '';
            $notification_data['cancellation_date'] = isset($data['cancellation_date']) ? $data['cancellation_date'] : '';
            ...
            $notification_data['verification_link'] = isset($data['verification_link']) ? $data['verification_link'] : '';
            $notification_data['shop_id'] = isset($data['shop_id']) ? $data['shop_id'] : '';
            $notification_data['shop_name'] = isset($data['shop_name']) ? $data['shop_name'] : '';
            $notification_data['shop_status'] = isset($data['shop_status']) ? $data['shop_status'] : '';
            $notification_data['shop_address'] = isset($data['shop_address']) ? $data['shop_address'] : '';
            $notification_data['shop_registration'] = isset($data['shop_registration']) ? $data['shop_registration'] : '';
            $notification_data['shop_city'] = isset($data['shop_city']) ? $data['shop_city'] : '';
            $notification_data['shop_state'] = isset($data['shop_state']) ? $data['shop_state'] : '';
            $notification_data['shop_country'] = isset($data['shop_country']) ? $data['shop_country'] : '';
            $notification_data['shop_contact'] = isset($data['shop_contact']) ? $data['shop_contact'] : '';
            $notification_data['shop_email'] = isset($data['shop_email']) ? $data['shop_email'] : '';
            $notification_data['provider_id'] = isset($data['provider_id']) ? $data['provider_id'] : '';
            $notification_data['provider_name'] = isset($data['provider_name']) ? $data['provider_name'] : (isset($data['provider_id']) ? (optional(User::find($data['provider_id']))->display_name ?? '') : '');
        ...
        if ($mailable != null && $mailable->to != null) {
            ...
            foreach ($mails as $key => $mailTo) {
                if ($notification_type === 'email_verification' && !empty($verificationUserId)) {
                    \Log::info("email_verification running-----------------".$verificationUserId);
                    $targetUser = \App\Models\User::find($verificationUserId);
                    if ($targetUser && !empty($targetUser->email)) {
                        $notification_data['person_id'] = $targetUser->id;
                        $notification_data['user_type'] = $targetUser->user_type ?? ($data['user_type'] ?? 'user');
                        try {
                            \Log::info('try run===============');
                            $targetUser->notify(new \App\Notifications\CommonNotification($notification_type, $notification_data));
                        } catch (\Symfony\Component\Mailer\Exception\TransportExceptionInterface $e) {
                           \Log::info('TransportExceptionInterface run===============');
                            $targetUser->update(['is_email_verified' => 1]);
                        } catch (\Symfony\Component\Mime\Exception\LogicException $e) {
                           \Log::info('LogicException run===============');
                            $targetUser->update(['is_email_verified' => 1]);
                        } catch (\Exception $e) {
                            Log::error('Notification email failed (email_verification): ' . $e->getMessage(), [
                ...
                if (isset($data['notify_to']) && is_array($data['notify_to']) && !in_array($mailTo, $data['notify_to'])) {
                    continue;
                }
                switch ($mailTo) {
                    case 'admin':
                        ...
                        if (isset($admin->email)) {
                            try {
                                ...
                                Log::error('Notification email failed (admin): ' . $e->getMessage());
                            }
                        }
                            ...
                            foreach ($providerId as $id) {
                                ...
                                if (isset($employee->email)) {
                                    try {
                                        ...
                                        Log::error('Notification email failed (provider): ' . $e->getMessage(), [
                                            'user_id' => $employee->id,
                                        ...
                                        foreach ($handymanId as $id) {
                                            ...
                                            if (isset($employee->email) && $employee->user_type == 'handyman') {
                                                try {
                                                    ...
                                                    Log::error('Notification email failed (handyman): ' . $e->getMessage(), [
                                                        'user_id' => $employee->id,
                                                        'email' => $employee->email,
                                                        'notification_type' => $notification_type,
                                                        'error_type' => get_class($e)
                        ...
                        if (isset($data['userIds']) && is_array($data['userIds']) && !empty($data['userIds'])) {
                            foreach ($data['userIds'] as $uid) {
                                ...
                                if ($user && !empty($user->email)) {
                                    ...
                                    $notification_data['user_name'] = $user->display_name ?? '';
                                    try {
                                        $notification_data['user_type'] = $mailTo;
                                        $user->notify(new \App\Notifications\CommonNotification($notification_type, $notification_data));
                                    } catch (\Exception $e) {
                                        Log::error('Notification email failed (user): ' . $e->getMessage(), [
                                            'user_id' => $user->id,
                                            'email' => $user->email,
                                            'notification_type' => $notification_type,
                                            'error_type' => get_class($e)
    ...
    /**
     * Send forgot-password email using notification_templates / mail_templates (replaces Laravel default ResetPassword).
     */
    public function sendForgotPasswordNotificationForUser(User $user, string $token): void
    {
        ...
        $payload = [
            ...
            'person_id' => $user->id,
            'user_name' => $user->display_name ?? $user->first_name ?? $user->email,
        ...
        try {
            $user->notify(new \App\Notifications\CommonNotification('forgot_password', $payload));
        } catch (\Throwable $e) {
    ...
    /**
     * Send confirmation email after password was reset successfully.
     */
    public function sendPasswordResetSuccessNotificationForUser(User $user): void
    {
        $payload = [
            'user_type' => $user->user_type,
            'person_id' => $user->id,
        ...
        try {
            $user->notify(new \App\Notifications\CommonNotification('password_reset', $payload));
        } catch (\Throwable $e) {
            Log::error('Password reset success mail failed: ' . $e->getMessage(), [
                'user_id' => $user->id,
                'email' => $user->email,
            ]);
        }
