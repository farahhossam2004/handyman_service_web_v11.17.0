...
class CommonNotification extends Notification implements ShouldQueue
{
    ...
    public function __construct($type, $data)
    {
        ...
        if (!$templateData) {
            // Fallback to generic user mapping when exact role mapping does not exist (e.g. custom roles like "test")
            $templateData = $notify_data->where('user_type', 'user')->first();
        }

        $this->template_data = $templateData;
        $templateDetail = $templateData->template_detail ?? __('messages.default_notification_body');
        foreach ($this->data as $key => $value) {
            $templateDetail = str_replace('[[ ' . $key . ' ]]', $this->data[$key], $templateDetail);
        }
        $this->data['type'] = $templateData->subject ?? $this->type;
...
{
    $userType = strtolower($this->data['user_type'] ?? 'user');
    ...
    if ($mail) {
        ...
        $mailTemplateData = MailTemplateContentMapping::where('template_id', $mail->id)
            ->where('user_type', $userType)
            ->first();
        if (!$mailTemplateData && $userType !== 'user') {
            // Fallback for custom roles (e.g. "test"): reuse generic user template mapping
            $mailTemplateData = MailTemplateContentMapping::where('template_id', $mail->id)
        ...
        if ($mailTemplateData) {
            ...
            // Step 3: Get mail body – prefer current notification template content (already loaded for this type + user_type)
            $mailBody = null;
            if ($this->template_data) {
                $mailBody = $this->template_data->mail_template_detail ?? $this->template_data->template_detail ?? null;
            }
            if (empty($mailBody)) {
                $customTemplate = NotificationTemplateContentMapping::where('subject', $subject)
                    ->where('user_type', $userType)
                    ->first();
                if (!$customTemplate && $userType !== 'user') {
                    $customTemplate = NotificationTemplateContentMapping::where('subject', $subject)
                        ->where('user_type', 'user')
                        ->first();
                }
                if ($customTemplate) {
                    $mailBody = $customTemplate->mail_template_detail ?? $customTemplate->template_detail ?? null;
                }
            }
            if (empty($mailBody) && $this->template_data) {
                $mailBody = $this->template_data->mail_template_detail ?? $this->template_data->template_detail ?? __('messages.default_notification_body');
            }
            if (empty($mailBody)) {
                $mailBody = __('messages.default_notification_body');
            }
            foreach ($this->data as $key => $value) {
                if (is_string($value)) {
                    $mailBody = str_replace('[[ ' . $key . ' ]]', (string) $value, $mailBody);
                }
            }
    ...
    // Fallback: when mail_templates row is missing (e.g. after MailTemplateSeeder wipes table), use notification template content
    if (empty($this->subject) && $this->template_data) {
        Log::warning("Mail template not found for type: {$this->type}, using notification template content");
        $this->subject = $this->template_data->mail_subject ?? $this->template_data->subject ?? 'Notification';
        $mailBody = $this->template_data->mail_template_detail ?? $this->template_data->template_detail ?? __('messages.default_notification_body');
        foreach ($this->data as $key => $value) {
            if (is_string($value)) {
    ...
    $notification = $this->notification ?? (object)['bcc' => '[]', 'cc' => '[]'];
    return (new MailMailableSend($notification, $this->data, $this->type))
        ->to($email)
        ->bcc(json_decode($notification->bcc ?? '[]'))
        ->cc(json_decode($notification->cc ?? '[]'))
