...
class BookingDetailResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        // Net refund: 0 if advance <= cancellation fee; else positive (advance - fee), never negative.
        $netRefundAmount = 0;
        if ($this->status == 'cancelled' && $advancepaid > 0) {
            $netRefundAmount = max(0, $advancepaid - (float) $cacellationcharges);
        }
        return [
            'id'                 => $this->id,
            ...
            'redeemed_discount'  => $this->redeemed_discount,
            'status'             => $this->status,
            ...
            'payment_status'     => (optional($this->service)->price == 0 || $this->total_amount == 0) ? 'free' : (optional($this->payment)->payment_status ?? null),
            'payment_status_label' => booking_payment_status_label($this->payment, $this),
