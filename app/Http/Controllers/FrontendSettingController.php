...
class FrontendSettingController extends Controller
{
    ...
    public function layoutPage(Request $request)
    {
        ...
        $data = view('frontendsetting.' . $page, compact('landing_page_data', 'page', 'tabpage', 'user_data', 'language_array'))->render();
    ...
    public function landingpagesettingsUpdates(Request $request)
    {
        ...
        if ($status == 1) {
            ...
            if (in_array($type, ['section_1', 'section_5', 'section_6', 'section_7', 'section_8', 'section_9'])) {
                $rules['description'] = 'required|string';
            }
            if (in_array($type, ['section_1']) && (isset($data['enable_popular_services']) && $data['enable_popular_services'] == 'on')) {
                $rules['category_id'] = 'required|array';
                $rules['category_id.*'] = 'exists:categories,id';
            }
            if (in_array($type, ['section_1']) && (isset($data['enable_popular_provider']) && $data['enable_popular_provider'] == 'on')) {
                $rules['provider_id'] = 'required|array';
                $rules['provider_id.*'] = 'exists:users,id';
            }
            if (in_array($type, ['section_2'])) {
                $rules['category_id'] = 'required|array';
                $rules['category_id.*'] = 'exists:categories,id';
            }
            if (in_array($type, ['section_3', 'section_4'])) {
            ...
            $request->validate($rules, [
                ...
                'category_id.required' => __('validation.required', ['attribute' => __('messages.select_name', ['select' => __('messages.category')])]),
                'service_id.required' => __('validation.required', ['attribute' => __('messages.select_name', ['select' => __('messages.service')])]),
                'provider_id.required' => __('validation.required', ['attribute' => __('messages.select_name', ['select' => __('messages.provider')])]),
            ]);
        }
        ...
        switch($type) {
            case 'section_1':
                $translatable_attributes = ['title', 'description'];
                break;
            case 'section_2':
            ...
            case 'section_6':
                ...
                break;
            case 'section_7':
                $translatable_attributes = ['title', 'description', 'subtitle', 'subdescription'];
                break;
            case 'section_8':
                $translatable_attributes = ['title', 'description'];
                break;
        ...
        if ($res) {
            $res->saveTranslations($data, $translatable_attributes, $language_option, 'en');
        }
    ...
    public function landingLayoutPage(Request $request)
    {
        ...
        switch ($tabpage) {
            case 'section_1':
                ...
                $data  = view('forntend-setting-landing.' . $tabpage, compact('user_data', 'tabpage', 'landing_page', 'language_array'))->render();
