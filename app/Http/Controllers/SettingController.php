...
class SettingController extends Controller
{
        ...
        switch ($page) {
            ...
            case 'service-configurations':
                ...
                if (!empty($serviceconfig['value'])) {
                    ...
                    $keys = ['advance_payment', 'slot_service', 'digital_services', 'service_packages', 'service_addons', 'post_services', 'shop', 'global_advance_payment', 'advance_paynment_percantage', 'cancellation_charge', 'cancellation_charge_amount', 'cancellation_charge_hours'];
                    foreach ($keys as $key) {
            ...
            case 'notification-setting':
            // Query Notification Templates and other data as you did before
            $query_data = NotificationTemplate::with('defaultNotificationTemplateMap', 'constant')->get();
            $data = [];
            ...
            // Fetch 'other-setting' values
            $othersetting = Setting::where('type', '=', 'OTHER_SETTING')->first();
            if (!empty($othersetting['value'])) {
                $decodedata = json_decode($othersetting['value']);

                // Keys for the boolean flags
                $booleanKeys = [
                    'whatsapp_notification',  // WhatsApp Notification flag
                    'sms_notification',       // SMS Notification flag
                ];

                // Process boolean keys (0 or 1)
                foreach ($booleanKeys as $key) {
                    $othersetting[$key] = isset($decodedata->$key) ? (int)$decodedata->$key : 0; // Default to 0
                }
            }

            // Return view with the updated data and other settings
            $data = view('setting.' . $page, compact('page', 'data', 'notificationChannels', 'othersetting'))->render();
            break;
            ...
            case 'other-setting':
                ...
                // $data = view('setting.' . $page, compact('page', var_names: 'othersetting'))->render();
            ...
            case 'seo-setting':
                ...
                $data = view('setting.' . $page, compact('page', 'seosetting', 'primary_locale', 'language_array'))->render();
                break;
            case 'data-reset':
                $earningsetting   = Setting::where('type', '=', 'earning-setting')->first();
                $data  = view('setting.' . $page, compact('earningsetting', 'page'))->render();
    ...
    public function updateProfile(UserRequest $request)
    {
        ...
        // Handle known_languages - filter out empty values
        if (isset($request->known_languages) && is_array($request->known_languages)) {
            $data['known_languages'] = json_encode(array_values(array_filter($request->known_languages, function ($value) {
                return $value !== null && trim($value) !== '';
            })));
        } else {
            $data['known_languages'] = null;
        }
        
        $data['display_name'] =  $request->first_name . ' ' . $request->last_name;
    ...
    public function saveEarningTypeSetting(Request $request)
    {
        ...
        // Get current earning type before update
        $currentSetting = Setting::where('type', 'earning-setting')->where('key', 'earning-setting')->first();
        $currentEarningType = $currentSetting ? $currentSetting->value : 'commission';
        ...
        // When switching to commission mode, activate all services and handymen
        if ($request->earning_type == 'commission' && $currentEarningType !== 'commission') {
            // Activate all services
            \App\Models\Service::query()->update(['status' => 1]);
            
            // Activate all handymen that are related to providers
            \App\Models\User::where('user_type', 'handyman')
                ->whereNotNull('provider_id')
                ->update(['status' => 1]);
        }
        
        // When switching to subscription mode, enforce plan limits for services, handymen, and featured services
        if ($request->earning_type == 'subscription' && $currentEarningType !== 'subscription') {
             enforcePlanLimitsForAllProviders();
        }
        
        // Save auto_assign_free_plan setting
        if ($request->earning_type == 'subscription') {
            $otherSetting = Setting::where('type', 'OTHER_SETTING')->where('key', 'OTHER_SETTING')->first();
            $otherSettingValue = $otherSetting ? json_decode($otherSetting->value, true) : [];
            
            $otherSettingValue['auto_assign_free_plan'] = $request->has('auto_assign_free_plan') ? 1 : 0;
            
            Setting::updateOrCreate(
                ['type' => 'OTHER_SETTING', 'key' => 'OTHER_SETTING'],
                ['value' => json_encode($otherSettingValue)]
            );
        }
    ...
    public function comission_handyman($id)
{
    $auth_user = authSession();  // Get the currently authenticated user

    // Check if the user is trying to access their own data or if they are an admin/demo_admin
    if ($id != auth()->user()->id && !auth()->user()->hasRole(['admin', 'demo_admin'])) {
        return redirect(route('home'))->withErrors(trans('messages.demo_permission_denied'));
    }

    // Fetch handyman data, using the `handymantype` relationship
    $handymandata = User::with('handymantype')->where('user_type', 'handyman')->where('id', $id)->first();

    // If handyman data doesn't exist, return an error message
    ...
    $pageTitle = __('messages.view_form_title', ['form' => __('messages.handyman')]);

    // Return the view with the necessary data
    return view('setting.comission_handyman', compact('pageTitle', 'handymandata', 'auth_user'));
}
    ...
    public function otherSetting(Request $request)
    {
        ...
        // Get existing OTHER_SETTING to preserve auto_assign_free_plan
        $existingSetting = Setting::where('type', 'OTHER_SETTING')->where('key', 'OTHER_SETTING')->first();
        $existingValue = $existingSetting ? json_decode($existingSetting->value, true) : [];
        $existingAutoAssignFreePlan = $existingValue['auto_assign_free_plan'] ?? 0;
        ...
        $other_setting_data = [
            ...
            'nearby_provider' => (isset($data['nearby_provider']) && $data['nearby_provider'] == 'on') ? 1 : 0,
            ...
            'auto_assign_free_plan' => $existingAutoAssignFreePlan, // Preserve the existing value
    ...
    public function seoSetting(SeoSettingRequest $request)
    {
        ...
        try {
            ...
            $primary_locale = app()->getLocale() ?? 'en';
            $translatableAttributes = ['meta_title', 'meta_description', 'meta_keywords'];
            $seosetting = [
            ...
            if (isset($data['translations']) && is_array($data['translations'])) {
                foreach ($data['translations'] as $lang => $fields) {
                    if (isset($fields['meta_keywords']) && !empty($fields['meta_keywords'])) {
                        $decoded = json_decode($fields['meta_keywords'], true);
                        if (is_array($decoded)) {
                            $keywordsArray = array_column($decoded, 'value');
                            // Store as JSON string so it gets saved as text in translations table
                            $data['translations'][$lang]['meta_keywords'] = json_encode($keywordsArray);
                        }
                    }
                }
            }
            $seoSetting->saveTranslations($data, $translatableAttributes, $language_option, $primary_locale);
    ...
    public function themeSetup(Request $request)
    {
        ...
        // Add cache buster timestamp to force browser to reload images
        $values['cache_buster'] = time();
        ...
        // Clear the image session cache so the new logo/favicon is loaded
        \Session::forget('images_data');
        
        // Clear all caches to ensure logo updates are reflected immediately
        \Artisan::call('cache:clear');
    ...
    public function serviceConfig(Request $request)
    {
        ...
        $serviceconfig = [
            ...
            'shop' => (isset($data['shop']) && $data['shop'] == 'on') ? 1 : 0,
    ...
    public function filterRevenue(Request $request)
{
    ...
    switch($type) {
        case 'year':
            $query->selectRaw('YEAR(date) as year, SUM(total_amount) as total')
                  ->groupBy('year');
            $results = $query->pluck('total', 'year')->toArray();

            $startYear = min(array_keys($results)) ?? date('Y') - 5;
            $endYear = max(array_keys($results)) ?? date('Y');
        ...
        case 'week':
            // --- NEW: weeks per month instead of actual revenue ---
            if($request->get('weeks_per_month')) {
                for($m=1;$m<=12;$m++){
                    $startOfMonth = \Carbon\Carbon::create($year, $m, 1)->startOfMonth();
                    $endOfMonth = $startOfMonth->copy()->endOfMonth();

                    // ISO week number calculation
                    $weeksInMonth = $startOfMonth->isoWeek() <= $endOfMonth->isoWeek()
                        ? $endOfMonth->isoWeek() - $startOfMonth->isoWeek() + 1
                        : $endOfMonth->isoWeek() + 52 - $startOfMonth->isoWeek() + 1;

                    $chartLabels[] = $startOfMonth->format('M');
                    $chartData[] = $weeksInMonth;
                }
            } else {
                // Existing revenue per week
                $query->selectRaw('WEEK(date, 1) as week, SUM(total_amount) as total') // week starts from Monday
                      ->whereYear('date', $year)
                      ->groupBy('week');
                $results = $query->pluck('total', 'week')->toArray();
        ...
        case 'custom':
            ...
            $query->selectRaw('DATE(date) as date, SUM(total_amount) as total')
                  ->groupBy('date');
            $results = $query->pluck('total', 'date')->toArray();
            ...
            if($start_date && $end_date){
                ...
                foreach($period as $date){
                    ...
                    $chartLabels[] = $label;
                    $chartData[] = $results[$date->format('Y-m-d')] ?? 0;
                }
            }
            break;

        default: // monthly
            $query->selectRaw('MONTH(date) as month, SUM(total_amount) as total')
                  ->whereYear('date', $year);
            if ($month && $month != 'all') {
