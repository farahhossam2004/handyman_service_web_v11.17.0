...
class NotificationTemplatesController extends Controller
{
    ...
    public function create()
    {
        ...
        // Get WhatsApp and SMS notification status from other settings
        ...
        if (!empty($othersetting['value'])) {
            ...
            $whatsapp_enabled = isset($decodedata->whatsapp_notification) ? (bool)$decodedata->whatsapp_notification : false;
            $sms_enabled = isset($decodedata->sms_notification) ? (bool)$decodedata->sms_notification : false;
        }

        return view('notificationtemplates.create', compact('module_action', 'assets', 'whatsapp_enabled', 'sms_enabled'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Clear WhatsApp and SMS template data if disabled
        if (!$whatsapp_enabled) {
            $map['whatsapp_subject'] = null;
            $map['whatsapp_template_detail'] = null;
        }

        if (!$sms_enabled) {
            $map['sms_subject'] = null;
            $map['sms_template_detail'] = null;
        }
    ...
    public function edit($id)
    {
        ...
        return view('notificationtemplates.form', compact('module_action', 'data', 'assets', 'buttonTypes', 'pageTitle', 'pageTitle1', 'page','notification_template_data','whatsapp_enabled','sms_enabled'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('defaultNotificationTemplateMap')) {
            ...
            if ($check !== null) {
                ...
                // Filter out WhatsApp and SMS template data if disabled
                $templateData = $request->defaultNotificationTemplateMap;
                if (!$whatsapp_enabled) {
                    $templateData['whatsapp_subject'] = null;
                    $templateData['whatsapp_template_detail'] = null;
                }

                if (!$sms_enabled) {
                    $templateData['sms_subject'] = null;
                    $templateData['sms_template_detail'] = null;
                }

                $data->defaultNotificationTemplateMap()->create($templateData);
