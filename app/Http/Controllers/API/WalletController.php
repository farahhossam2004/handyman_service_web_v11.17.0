...
class WalletController extends Controller
{
    ...
    public function getHistory(Request $request)
    {
        ...
        $wallet_history = WalletHistory::with('providers')->where('user_id', $user_id)
                    ->where('activity_data', 'NOT LIKE', '%"payment_method":"cash"%');

        if ($user && $user->user_type == 'handyman') {
            $wallet_history = $wallet_history->whereNot('activity_type','wallet_payout_transfer');
        }
    ...
    public function store(Request $request)
    {
        ...
        // UI "edit wallet" sends the current amount again (not a delta).
