...
class DashboardController extends Controller
{
    ...
    {
        \Log::info('Dashboard Detail Request', [
            'all_params' => $request->all(),
            'latitude' => $request->latitude,
            'longitude' => $request->longitude,
            'has_latitude' => $request->has('latitude'),
            'has_longitude' => $request->has('longitude'),
            'latitude_empty' => empty($request->latitude),
            'longitude_empty' => empty($request->longitude)
        ]);
        ...
        $category = CategoryResource::collection(
            Category::whereIN('id', $category_section->category_id)->where('status', 1)
                ->orderBy('name', 'asc')
                ->paginate(8)
        );

        // Get all services (not filtered by section_3 settings)
        // Initialize service query - show ALL services
        $serviceQuery = Service::where('status', 1)
            ->where('service_type', 'service')
            ->where('service_request_status', "approve");
        
        \Log::info('Dashboard Detail - Initial service count before filters:', [
            'count' => $serviceQuery->count()
        ]);


        if (!(auth()->user() !== null && (auth()->user()->hasRole('admin') || auth()->user()->hasRole('provider')))) {
            $serviceQuery->where('status', 1);
        }
        ...
        // Check if location is explicitly enabled by the user
        $locationEnabled = $request->has('latitude') && !empty($request->latitude) && 
                          $request->has('longitude') && !empty($request->longitude);

        \Log::info('Dashboard Detail Location Check', [
            'location_enabled_param' => $request->location_enabled ?? 'not set',
            'has_latitude' => $request->has('latitude'),
            'has_longitude' => $request->has('longitude'),
            'locationEnabled' => $locationEnabled,
            'serviceZone_count' => count($serviceZone)
        ]);

      
        // Apply zone-based location filter when coordinates are provided (location is turned ON)
        if ($locationEnabled) {
            ...
            if (count($serviceZone) > 0) {
                ...
                try {
                    ...
                    \Log::info('Dashboard Detail Matching Zones', [
                    ...
                    if (!empty($matchingZoneIds)) {
                        ...
                        \Log::info('Dashboard Detail - Applied zone filter to services', [
                            'count_after_zone_filter' => $serviceQuery->count()
                        ]);
                    } else {
                        // If no zones match, return empty results
                        $serviceQuery->whereRaw('1 = 0');
                        \Log::info('Dashboard Detail - No matching zones, returning empty services');
                    }
                } catch (\Exception $e) {
                    \Log::error('Location filtering error: ' . $e->getMessage());
                }
            } else {
                \Log::info('Dashboard Service Filter - No zones configured, showing all services');
            }
        } else {
            \Log::info('Dashboard Service Filter - Location OFF, showing all services');
        }
        ...
        // Order by latest services
        $serviceQuery->orderBy('id', 'desc');

        // Get final services - paginate AFTER all filters are applied
        $service = ServiceResource::collection(
            $serviceQuery->paginate($per_page)
        );
        ...
        // Apply location filter if coordinates are provided (location is turned ON)
        if ($locationEnabled) {
            try {
        ...
        // Fetch featured services based on section_4 settings (only services selected in admin panel)
        $featured_service_section = FrontendSetting::getValueByKey('section_4');
        
        // Log the service IDs for debugging
       

        $featured_service = Service::whereIn('id', $featured_service_section->service_id ?? [])->where('status',1);
        ...
        $featured_service = $featured_service->whereHas('providers', function ($a) use ($request) {
            ...
            \Log::info('Subscription filter NOT applied (earning type is commission)');
        }

        // Apply zone-based location filter for featured services when location is ON
        if ($locationEnabled && count($serviceZone) > 0) {
            try {
                $matchingZoneIds = $this->getMatchingZonesByLatLng($request->latitude, $request->longitude);
                \Log::info('Featured services - matching zone IDs:', ['zones' => $matchingZoneIds]);

                if (!empty($matchingZoneIds)) {
                    $featured_service->whereHas('serviceZoneMapping', function ($query) use ($matchingZoneIds) {
                        $query->whereIn('zone_id', $matchingZoneIds);
                    });
                    \Log::info('Featured services after zone filter:', ['count' => $featured_service->count()]);
                } else {
                    // If no zones match, return empty featured services
                    $featured_service->whereRaw('1 = 0');
                    \Log::info('Featured services - no matching zones, returning empty');
                }
            } catch (\Exception $e) {
                \Log::error('Featured service location filtering error: ' . $e->getMessage());
            }
        } else {
            \Log::info('Featured services - location filter skipped', [
                'locationEnabled' => $locationEnabled,
                'serviceZone_count' => count($serviceZone)
            ]);
        }

        // Preserve the order of services as selected in admin panel
        if (!empty($featured_service_section->service_id)) {
            $serviceIds = implode(',', $featured_service_section->service_id);
            $featured_service->orderByRaw("FIELD(id, {$serviceIds})");
        }

        \Log::info('Featured services final count before get:', ['count' => $featured_service->count()]);
        \Log::info('Featured services IDs:', ['ids' => $featured_service->pluck('id')->toArray()]);

        $featured_service = ServiceResource::collection($featured_service->get());

        // NEW: Get ALL services in the zone (not just featured)
        // This section shows all available services when location is ON
        $all_services_query = Service::where('status', 1)
            ->where('service_type', 'service')
            ->where('service_request_status', "approve");

        // Filter by provider status
        $all_services_query->whereHas('providers', function ($query) {
            $query->where('status', 1);
        });
        ...
        // Apply zone-based location filter when location is ON
        if ($locationEnabled && count($serviceZone) > 0) {
            try {
                $matchingZoneIds = $this->getMatchingZonesByLatLng($request->latitude, $request->longitude);

                if (!empty($matchingZoneIds)) {
                    $all_services_query->whereHas('serviceZoneMapping', function ($query) use ($matchingZoneIds) {
                        $query->whereIn('zone_id', $matchingZoneIds);
                    });
                    \Log::info('Dashboard Detail - All services zone filter applied', [
                        'count' => $all_services_query->count()
                    ]);
                } else {
                    // If no zones match, return empty results
                    $all_services_query->whereRaw('1 = 0');
                    \Log::info('Dashboard Detail - No matching zones, returning empty all services');
                }
            } catch (\Exception $e) {
                \Log::error('All services location filtering error: ' . $e->getMessage());
            }
        }

        // Apply city filter if provided
        if ($request->has('city_id') && !empty($request->city_id)) {
            $all_services_query->whereHas('providers', function ($query) use ($request) {
                $query->where('city_id', $request->city_id);
            });
        }

        // Order by latest
        $all_services_query->orderBy('id', 'desc');

        $all_services = ServiceResource::collection($all_services_query->paginate($per_page));
        ...
        $blogs = BlogResource::collection(Blog::where('status', 1)->paginate($per_page));
        ...
        // Apply location-based filter when location is ON
        if ($locationEnabled) {
            try {
                $get_distance = getSettingKeyValue('site-setup', 'radious');
                $get_unit = getSettingKeyValue('site-setup', 'distance_type');
        ...
        if (default_earning_type() === 'subscription') {
            $shopQuery = $shopQuery->whereHas('provider', function ($query) {
                $query->where('status', 1)->where('is_subscribe', 1);
            });
        }
    ...
    public function providerDashboard(Request $request)
    {
        ...
        $has_active_resources = \App\Models\Service::where('provider_id', $provider->id)
        ->where('status', 1)->exists() || 
        \App\Models\User::where('provider_id', $provider->id)
        ->where('status', 1)
        ->exists();
        if(!$has_active_resources){
            $downgrade_banner = get_provider_downgrade_banner($provider->id);
            
            // Get upgrade banner if applicable
            $upgrade_banner = get_provider_upgrade_banner($provider->id);
            // Get downgrade banner if applicable
        }else{
            $downgrade_banner = null;
            $upgrade_banner = null;
        }
        ...
        $response = [
            ...
            'downgrade_banner' => $downgrade_banner,
    ...
    public function configurations(Request $request)
    {
        ...
        // "shop" should follow module setting from service configuration.
        $roleAndPermission['shop'] = isset($service_config->shop) ? (int) $service_config->shop : 0;
        ...
        $response = [
            ...
            "default_latitude" => isset($sitesetup->latitude) ? (float) $sitesetup->latitude : null,
            "default_longitude" => isset($sitesetup->longitude) ? (float) $sitesetup->longitude : null,
            ...
            "is_nearby_provider_enable" => isset($other_setting->nearby_provider) ? (int)$other_setting->nearby_provider : 0,
