                ...
                case 'airtel':
                    ...
                    case 'phonepe':
                        if(!empty($payment_data['value'])){
                            ...
                            $payment_data['phonepe_version'] = $decodedata['phonepe_version'] ?? 'v1';
                            $payment_data['merchant_id_v1'] = $decodedata['merchant_id'] ?? $decodedata['merchant_id_v1'] ?? '';
                            $payment_data['salt_key_v1'] = $decodedata['salt_key'] ?? $decodedata['salt_key_v1'] ?? '';
                            $payment_data['salt_index_v1'] = $decodedata['salt_index'] ?? $decodedata['salt_index_v1'] ?? '';
                            $payment_data['merchant_id_v2'] = $decodedata['merchant_id_v2'] ?? '';
                            $payment_data['client_id_v2'] = $decodedata['client_id_v2'] ?? '';
                            $payment_data['client_secret_v2'] = $decodedata['client_secret_v2'] ?? '';
                        }
                        if(!empty($payment_data['live_value'])){
                            $liveDecodedata = json_decode($payment_data['live_value'], true);
                            $payment_data['live_phonepe_version'] = $liveDecodedata['phonepe_version'] ?? 'v1';
                            $payment_data['live_merchant_id_v1'] = $liveDecodedata['merchant_id'] ??  $liveDecodedata['merchant_id_v1'] ?? '';
                            $payment_data['live_salt_key_v1'] = $liveDecodedata['salt_key'] ?? $liveDecodedata['salt_key_v1'] ?? '';
                            $payment_data['live_salt_index_v1'] = $liveDecodedata['salt_index'] ?? $liveDecodedata['salt_index_v1'] ?? '';
                            $payment_data['live_merchant_id_v2'] = $liveDecodedata['merchant_id_v2'] ?? '';
                            $payment_data['live_client_id_v2'] = $liveDecodedata['client_id_v2'] ?? '';
                            $payment_data['live_client_secret_v2'] = $liveDecodedata['client_secret_v2'] ?? '';
                        }
                        $data  = view('paymentgateway.'.$tabpage, compact('user_data','tabpage','payment_data'))->render();
                      break;
        ...
        switch ($type) {
            ...
            case 'sadad':
                ...
                case 'airtel':
                    ...
                    case 'phonepe':
                        $config_data = [
                            'phonepe_version' => $data['phonepe_version'],
                            'merchant_id_v1' => $data['merchant_id_v1'] ?? $data['merchant_id'] ?? '',
                            'salt_key_v1' => $data['salt_key_v1'] ?? $data['salt_key'] ?? '',
                            'salt_index_v1' => $data['salt_index_v1'] ?? $data['salt_index'] ?? '',
                            'merchant_id_v2' => $data['merchant_id_v2'] ?? '',
                            'client_id_v2' => $data['client_id_v2'] ?? '',
                            'client_secret_v2' => $data['client_secret_v2'] ?? '',
                        ];
                    break;
                    case 'midtrans':
                        $config_data = [
